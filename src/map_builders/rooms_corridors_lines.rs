...
impl StraightLineCorridors {
    ...
    fn corridors(&mut self, build_data: &mut BuilderMap) {
        ...
        for (i, room) in rooms.iter().enumerate() {
            ...
            let room_center_pt = bracket_lib::prelude::Point::new(room_center.0, room_center.1);
            for (j, other_room) in rooms.iter().enumerate() {
                if i != j && !connected.contains(&j) {
                    let other_center = other_room.center();
                    let other_center_pt =
                        bracket_lib::prelude::Point::new(other_center.0, other_center.1);
                    let distance = bracket_lib::prelude::DistanceAlg::Pythagoras
                        .distance2d(room_center_pt, other_center_pt);
            ...
            if !room_distance.is_empty() {
                ...
                let line = bracket_lib::prelude::line2d(
                    bracket_lib::prelude::LineAlg::Bresenham,
                    room_center_pt,
                    bracket_lib::prelude::Point::new(dest_center.0, dest_center.1),
                );
