...
pub fn town_builder(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    let mut chain = BuilderChain::new(new_depth, width, height, "The Town of Bree");
...
impl TownBuilder {
    ...
    pub fn build_rooms(&mut self, build_data: &mut BuilderMap) {
        ...
        self.building_factory(build_data, &buildings, &building_size, wall_gap_y);
    ...
    fn building_factory(
        ...
        for (i, _size, build_type) in building_index.iter() {
            ...
            match build_type {
                BuildingTag::Pub => self.build_pub(&building, build_data, wall_gap_y),
                BuildingTag::Temple => self.build_temple(&building, build_data, wall_gap_y),
                BuildingTag::Blacksmith => self.build_smith(&building, build_data, wall_gap_y),
                BuildingTag::Clothier => self.build_clothier(&building, build_data, wall_gap_y),
                BuildingTag::Alchemist => self.build_alchemist(&building, build_data, wall_gap_y),
                BuildingTag::PlayerHouse => self.build_my_house(&building, build_data, wall_gap_y),
                BuildingTag::Hovel => self.build_hovel(&building, build_data, wall_gap_y),
    ...
    fn random_building_spawn(
        &mut self,
        // A tuple representing the building's dimensions and position:
        // x and y are the coordinates of the top-left corner.
        // width and height define the building's size.
        building: &(i32, i32, i32, i32),
        build_data: &mut BuilderMap,
        entities_to_place: &mut Vec<&str>,
        player_idx: usize,
        wall_gap_y: i32,
    ) {
        let max_attempts = 1000;
        let cy = building.1 + (building.3 / 2);
        // attempt to avoid infinite loop
        let mut current_attempt = 0;
        while !entities_to_place.is_empty() || current_attempt >= max_attempts {
            current_attempt += 1;
            ...
            if build_data.map.tiles[idx] == TileType::WoodFloor
                ...
                && !entities_to_place.is_empty()
            {
                let entity_tag = entities_to_place[0];
                entities_to_place.remove(0);
    ...
    fn build_pub(
        ...
        self.random_building_spawn(building, build_data, &mut to_place, player_idx, wall_gap_y);
    }
    ...
    fn build_temple(
        ...
        self.random_building_spawn(building, build_data, &mut to_place, 0, wall_gap_y);
    }
